...
for child_folder in footage_child_folders:
    ...
    audio = AudioFileClip(f'{audio_directory}/{random_audio_file}').set_duration(clip_duration).fx(afx.audio_fadeout, 2)
    ...
    clip15 = VideoFileClip(video_clips_list[14], fps_source="fps").subclip(2, 4)
    clip16 = VideoFileClip(video_clips_list[15], fps_source="fps").subclip(2, 4)
    clip17 = VideoFileClip(video_clips_list[16], fps_source="fps").subclip(2, 5)

    # combine video & audio files and export
    combined = concatenate_videoclips([bumper_in, clip1, clip2, clip3, clip4, clip5, clip6, clip7, clip8, clip9, clip10, clip11, clip12, clip13, clip14, clip15, clip16, clip17, bumper_out])
